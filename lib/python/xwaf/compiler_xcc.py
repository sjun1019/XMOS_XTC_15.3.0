...
@extension('.xscope')
def xscope_hook(self, xscope_source_file_node):
    class Xscope_Task(Task):
        always_run = True

        def run(self):
            return self.exec_command('echo %s' % (self.outputs[0].abspath()))

    task = self.create_task("Xscope_Task", None, xscope_source_file_node)

    # Append to compiled tasks (list may not exist yet)
    # Pattern copied from ccroot.create_compiled_task
    try:
