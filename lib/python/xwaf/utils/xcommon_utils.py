...
def parse_makefile(node, makefile, xcc_flags=None):
    ...
    mf_parseable = node.make_node(
            "{}_parseable_{}_{}".format(makefile, random.randint(0, 2**32), os.getpid())
    )
...
def read_makefiles(app, config, makefile_contents=None):
    ...
    for variable in app.makefile_contents:
        if variable.startswith('XCC_FLAGS_') and not (variable.endswith('.c') or variable.endswith('.xc')):
    ...
    # Look for config.xscope in project root
    root_xscope = app.find_resource("config.xscope")

    if root_xscope is not None and root_xscope not in app.source_nodes:
        app.source_nodes.append(root_xscope)

    # Check that there's only one xscope config
    xscope_files = [n for n in app.source_nodes if n.name.endswith('.xscope')]

    if len(xscope_files) > 1:
        raise WafError("More than one xscope config found: " + str(xscope_files))
...
def apply_source_nodes(app_or_module, existing_xcc_flags):
    ...
    for raw_source_dir in raw_source_dirs:
        ...
        source_dirs.append(raw_source_dir + '/*.xscope')
    ...
    for node in source_nodes:
        xcc_flag_key_list = [key for key, value in makefile_contents.items() if key.startswith('XCC_FLAGS') and key.endswith(str(node.name))]
        for variable in xcc_flag_key_list:
            ...
            local_xcc_flags = XCC_FLAGS[:]
            local_existing_xcc_flags = existing_xcc_flags[:]
            local_xcc_flags.reverse()
            local_existing_xcc_flags.reverse()
            idx = 0
            while local_xcc_flags[idx] == local_existing_xcc_flags[idx]:
                idx += 1
            local_xcc_flags.reverse()
            local_existing_xcc_flags.reverse()
            del local_xcc_flags[-idx:]
            del local_existing_xcc_flags[-idx:]
            
            if local_xcc_flags[:len(local_existing_xcc_flags)] == local_existing_xcc_flags:
                # User has acknowledged that XCC_FLAGS (or MODULE_XCC_FLAGS in the case of a module) will be applied
                # We remove the unnecessary additional flags
                node.xccflags = XCC_FLAGS[len(local_existing_xcc_flags):]
                break
        else:   # Case met when for loop completes without a break
            if xcc_flag_key_list:
                error(
                    "%r sets variable(s) %r, but none include ${XCC_FLAGS} or ${MODULE_XCC_FLAGS} (as appropriate) as the first part of the variable."
                    % (app_or_module.name, xcc_flag_key_list))
